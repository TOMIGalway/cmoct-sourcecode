...
public class FourierTransform3short
{
	...
	public FourierTransform3short(int dataSize)
	{
		// find power of two greater than num of points in fdata
		int nPointsPow2 = 1;
		while (nPointsPow2 < dataSize)
			nPointsPow2 *= 2;
		fdata = new short[nPointsPow2 * 2];
	}

	public void fft(short[] reIn, short[] imIn, short[] reOut, short[] imOut)
	{
		fft(reIn, imIn, reOut, imOut, null, null);
	}

	public void fft(short[] reIn, short[] reOut, short[] imOut)
	{
	...
	public void fft(short[] reIn, short[] imIn, short[] reOut, short[] imOut, short[] mag, short[] phase)
	{
		...
		for (i = 0, j = 0; i < reOut.length; i++)
		{
			...
			if (mag != null)
			{
				mag[i] = (short) (2 * (Math.sqrt(reOut[i] * reOut[i] + imOut[i]
						* imOut[i])) / reIn.length);
			}

			if (phase != null)
			{
				phase[i] = (short) Math.atan2(imOut[i], reOut[i]);
			}
		}
	}
	 ...
	 */
	public void four1(short[] data, int isign)
	{
		...
		while (n > mmax)
		{
			...
			for (m = 0; m < mmax; m += 2)
			{
				for (i = m; i < n; i += istep)
				{
					...
					data[j] = (short) (data[i] - tempr);
					data[j + 1] = (short) (data[i + 1] - tempi);
					data[i] += tempr;
					data[i + 1] += tempi;
				}

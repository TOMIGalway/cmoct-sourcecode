...
public class SlicePlane2DRenderer extends SlicePlaneRenderer implements
		VolRendConstants
{
	...
	private void tesselateTri(Group polyGroup, Point3d pt0, Point3d pt1, Point3d pt2)
	{
		 ...
		 */
		intersect(midCalc, pt[bot], rCoord[bot], pt[top], rCoord[top], rCoord[mid]);
		...
		if (midIsRight)
		{
			...
			{
				tesselateBottomTri(triGroup, orderDir, tg, textures, pt[bot], rCoord[bot], midCalc, pt[mid], rCoord[mid]);
			}
			if (rCoord[mid] < rCoord[top])
			{
				tesselateTopTri(triGroup, orderDir, tg, textures, midCalc, pt[mid], rCoord[mid], pt[top], rCoord[top]);
			}
		} else
		{
			if (rCoord[bot] < rCoord[mid])
			{
				tesselateBottomTri(triGroup, orderDir, tg, textures, pt[bot], rCoord[bot], pt[mid], midCalc, rCoord[mid]);
			}
			if (rCoord[mid] < rCoord[top])
			{
				tesselateTopTri(triGroup, orderDir, tg, textures, pt[mid], midCalc, rCoord[mid], pt[top], rCoord[top]);
			}
		}
	...
	private void tesselateTopTri(OrderedGroup triGroup, int orderDir, TexCoordGeneration tg, Texture2D[] textures, Point3d midLeft, Point3d midRight, double rMid, Point3d top, double rTop)
	{
		...
		if (upperIndex == lowerIndex)
		{
			...
			outputTopTri(triGroup, orderDir, tg, textures, lowerIndex, midLeft, midRight, lowerWeight, top, upperWeight);
		} else
		{
			if (verbose)
			{
			...
			intersect(upperLeftCut, midLeft, rMid, top, rTop, (double) lowerIndex + 1);
			intersect(upperRightCut, midRight, rMid, top, rTop, (double) lowerIndex + 1);
			if (verbose)
			{
			...
			// output quad
			outputQuad(triGroup, orderDir, tg, textures, lowerIndex, midLeft, midRight, lowerWeight, upperLeftCut, upperRightCut, 1.0);
			...
			if ((upperIndex - lowerIndex) > 1)
			{
				...
				outputCutQuads(triGroup, orderDir, tg, textures, lowerIndex + 1, upperIndex, midLeft, midRight, rMid, top, top, rTop);
			}
			...
			intersect(lowerLeftCut, midLeft, rMid, top, rTop, upperIndex);
			intersect(lowerRightCut, midRight, rMid, top, rTop, upperIndex);
			if (verbose)
			{
			...
			outputTopTri(triGroup, orderDir, tg, textures, upperIndex, lowerLeftCut, lowerRightCut, 0.0, top, upperWeight);
		}

	}

	private void tesselateBottomTri(OrderedGroup triGroup, int orderDir, TexCoordGeneration tg, Texture2D[] textures, Point3d bottom, double rBottom, Point3d midLeft, Point3d midRight, double rMid)
	{
		...
		if (upperIndex == lowerIndex)
		{
			...
			outputBottomTri(triGroup, orderDir, tg, textures, lowerIndex, bottom, lowerWeight, midLeft, midRight, upperWeight);
		} else
		{
			if (verbose)
			{
			...
			// at least one cut. Figure out lower cut point, output bottom
			intersect(upperLeftCut, bottom, rBottom, midLeft, rMid, (double) lowerIndex + 1);
			intersect(upperRightCut, bottom, rBottom, midRight, rMid, (double) lowerIndex + 1);
			...
			outputBottomTri(triGroup, orderDir, tg, textures, lowerIndex, bottom, lowerWeight, upperLeftCut, upperRightCut, 1.0);
			...
			{
				// output middle cut quads
				outputCutQuads(triGroup, orderDir, tg, textures, lowerIndex + 1, upperIndex, bottom, bottom, rBottom, midLeft, midRight, rMid);
			}
			...
			intersect(lowerLeftCut, bottom, rBottom, midLeft, rMid, upperIndex);
			intersect(lowerRightCut, bottom, rBottom, midRight, rMid, upperIndex);
			if (verbose)
			{
				System.out.println("lowerLeftCut = " + lowerLeftCut);
				System.out.println("lowerRightCut = " + lowerRightCut);
			...
			// output quad
			outputQuad(triGroup, orderDir, tg, textures, upperIndex, lowerLeftCut, lowerRightCut, 0.0, midLeft, midRight, upperWeight);
		}

	}

	private void outputCutQuads(OrderedGroup triGroup, int orderDir, TexCoordGeneration tg, Texture2D[] textures, int lowerIndex, int upperIndex, Point3d bottomLeft, Point3d bottomRight, double rBottom, Point3d topLeft, Point3d topRight, double rTop)
	{
		...
		intersect(lowerLeftCut, bottomLeft, rBottom, topLeft, rTop, lowerIndex);
		intersect(lowerRightCut, bottomRight, rBottom, topRight, rTop, lowerIndex);
		if (verbose)
		{
			System.out.println("lowerLeftCut = " + lowerLeftCut);
			System.out.println("lowerRightCut = " + lowerRightCut);
		...
		{
			intersect(upperLeftCut, bottomLeft, rBottom, topLeft, rTop, (double) index + 1);
			intersect(upperRightCut, bottomRight, rBottom, topRight, rTop, (double) index + 1);
			if (verbose)
			{
				System.out.println("upperLeftCut = " + upperLeftCut);
				System.out.println("upperRightCut = " + upperRightCut);
			}
			outputQuad(triGroup, orderDir, tg, textures, index, lowerLeftCut, lowerRightCut, 0.0, upperLeftCut, upperRightCut, 1.0);
	...
	// on that edge.
	private void outputBottomTri(OrderedGroup triGroup, int orderDir, TexCoordGeneration tg, Texture2D[] textures, int lowerIndex, Point3d lower, double lowerWeight, Point3d upperLeft, Point3d upperRight, double upperWeight)
	{
		...
		outputShape(triGroup, orderDir, tg, textures[lowerIndex], 3, shapePts, shapeColrs);
	...
	private void outputTopTri(OrderedGroup triGroup, int orderDir, TexCoordGeneration tg, Texture2D[] textures, int lowerIndex, Point3d lowerLeft, Point3d lowerRight, double lowerWeight, Point3d upper, double upperWeight)
	{
	...
	private void outputQuad(OrderedGroup triGroup, int orderDir, TexCoordGeneration tg, Texture2D[] textures, int lowerIndex, Point3d lowerLeft, Point3d lowerRight, double lowerWeight, Point3d upperLeft, Point3d upperRight, double upperWeight)
	{

...
public class MultiSplitLayout implements LayoutManager
{
	...
	private Rectangle boundsWithYandHeight(Rectangle bounds, double y, double height)
	{
		...
		r
				.setBounds((int) (bounds.getX()), (int) y, (int) (bounds
						.getWidth()), (int) height);
		return r;
	}
	...
	private void layoutShrink(Split split, Rectangle bounds)
	{
		...
		if (split.isRowLayout())
		{
			...
			for (Node splitChild : split.getChildren())
			{
				...
				int nodeMinWidth = Math
						.min(nodeWidth, minimumNodeSize(splitChild).width);
		...
		else
		{
			...
			for (Node splitChild : split.getChildren())
			{
				...
				int nodeMinHeight = Math
						.min(nodeHeight, minimumNodeSize(splitChild).height);
	...
	private void layoutGrow(Split split, Rectangle bounds)
	{
		...
		if (split.isRowLayout())
		{
			...
			while (splitChildren.hasNext())
			{
				...
				{
					double allocatedWidth = (splitChild
							.equals(lastWeightedChild)) ? availableWidth : Math
		...
		else
		{
			...
			while (splitChildren.hasNext())
			{
				...
				{
					double allocatedHeight = (splitChild
							.equals(lastWeightedChild)) ? availableHeight

...
public class CPUCorrelationToolNew
{
	...
	public static void crossCorr(int inputData[], int inX, int inY, int sizeX, int sizeY, int threshold, int map[], int mapSize, float minCorr, float maxCorr, int out[])
	{
		int imgB = 1;
		int imgA = 0;
			...
			for (int yP = 0; yP < inY; yP++)
			{
				...
				for (int x = xP - sizeX; x <= xP + sizeX; x++)
				{
					for (int y = yP - sizeY; y <= yP + sizeY; y++)
					{
						...
						{
							gridA[pxlCount] = inputData[imgA * inX * inY
									+ inY * x + y];
							gridB[pxlCount] = inputData[imgB * inX * inY
									+ inY * x + y];
						} else
						{
							gridA[pxlCount] = 0;
				...
				float crossCorr = getCrossCorr(gridA, gridB, 2 * sizeX + 1, 2 * sizeY + 1, threshold);
				crossCorr = (crossCorr - minCorr) / (maxCorr - minCorr);
				if (crossCorr > 1)
				{
					crossCorr = 1;
				}
				...
				out[inY * xP + yP] = map[(int) ((mapSize -1)* crossCorr)];
			}
		}
	}

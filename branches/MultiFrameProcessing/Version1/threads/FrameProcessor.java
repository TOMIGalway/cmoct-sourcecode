...
public class FrameProcessor
{
	...
	public static void processScans(CrossCorrelationWorker worker)
	{
		byte[][] frmA = null;
		byte[][] frmB = null;
		
		int scanNumber = worker.master.task.getScanNumber();
		int frameNumber = worker.currentFrame;
		int startingPosition = frameNumber*scanNumber;
		int endingPosition = startingPosition + scanNumber;
		
		int combinationNumber = 0;
		int numberOfCombinations = getCombinations(scanNumber);
		
		int sizeX = worker.master.task.getSizeX();
		int sizeY = worker.master.task.getSizeY();

		float[][][] crossCorrelationValues = new float[numberOfCombinations][sizeX][sizeY];
		
		for(int i = startingPosition; i <= endingPosition; i++)
		{
			for(int j = (i+1); j < endingPosition; j++)
			{
				frmA = null;
				frmB = null;
				...
				try
				{
					frmA = worker.master.task.getFrame(i);
					frmB = worker.master.task.getFrame(j);
				} catch (IOException e)
				{
					System.out.println("Error processing frame : "
							+ worker.currentFrame);
				...
				System.out.println("Processing " + i + " and " + j);
					...
					for (int yP = 0; yP < worker.master.task.getSizeY(); yP++)
					{
								...
								+ worker.master.task.getKerX(); x++)
						{
							for (int y = yP - worker.master.task.getKerY(); y <= yP
									+ worker.master.task.getKerY(); y++)
							{
								// Check to see if this point on the kernel is within the bounds 
								// of the image.
								if (x < worker.master.task.getSizeX()
										&& y < worker.master.task.getSizeY() && x >= 0
										&& y >= 0)
						...
						crossCorrelationValues[combinationNumber][xP][yP] = worker.crossCorr;
					}
				}
				combinationNumber++;
			}
		...
		float averageCrossCorrValue = 0;
		float[][] averageCrossCorrValues = new float[worker.master.task.getSizeX()][worker.master.task.getSizeY()];
		
		for (int xP = 0; xP < worker.master.task.getSizeX(); xP++)
		{
			for (int yP = 0; yP < worker.master.task.getSizeY(); yP++)
			{
				float sumOfCrossCorrValues = 0;
				
				for(int comb = 0; comb < numberOfCombinations; comb++)
				{
					sumOfCrossCorrValues += crossCorrelationValues[comb][xP][yP];
				}
				averageCrossCorrValue = sumOfCrossCorrValues / numberOfCombinations;
				
				averageCrossCorrValues[xP][yP] = averageCrossCorrValue;
			}
		}
		...
		for (int xP = 0; xP < worker.master.task.getSizeX(); xP++)
		{
			for (int yP = 0; yP < worker.master.task.getSizeY(); yP++)
			{
				float crossCorrValue = averageCrossCorrValues[xP][yP];
				// DO MIP (Min Corellation
				// Projection)
				if (worker.master.task.isMIPinMemory())
				{
					if (((worker.master.task.MIPData[xP][worker.currentFrame] > crossCorrValue) && (yP >= worker.master.task
							.getMinPosMIP() && yP <= worker.master.task
							.getMaxPosMIP()))
							|| yP == worker.master.task.getMinPosMIP())
					{
						worker.master.task.MIPData[xP][worker.currentFrame] = crossCorrValue;
						worker.master.task.MIPDepth[xP][worker.currentFrame] = yP;
					}
				}
				/*
				 * Storing the raw cmOCT values
				 */
				if (worker.master.task.isCrossCorrRawInMemory())
				{
					worker.master.task.crossCorrRawData[worker.currentFrame][xP][yP] = (short) ((Short.MAX_VALUE) * crossCorrValue);
				}

				// Re map for byte Output data
				if (worker.master.task.isCrossCorrByteInMemory()
						|| worker.master.task.isSaveFlow())
				{
					crossCorrValue = (crossCorrValue - worker.master.task
							.getCrossCorrMin())
							/ (worker.master.task.getCrossCorrMax() - worker.master.task
									.getCrossCorrMin());
					if (crossCorrValue > 1)
				...
				if (worker.master.task.isSaveFlow())
				{
					worker.outFrameByte[yP * worker.master.task.getSizeX() + xP] = (byte) (255 * crossCorrValue);
				}

				if (worker.master.task.isCrossCorrByteInMemory())
				{
					worker.master.task.crossCorrByteData[worker.currentFrame][xP][yP] = (byte) (255 * crossCorrValue);
				}
			}		
		}
	...
	private static int getCombinations(int scanNumber)
	{
		return factorial(scanNumber) / (factorial(scanNumber - 2) * factorial(2));
	}
